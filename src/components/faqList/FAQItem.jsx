...
export const FAQItem = ({ item, onRemove, canRemove }) => {
  ...
  return (
    <div
      ...
      className=""
    >
      <button onClick={() => setIsExpanded(true)} className={`flex items-center justify-between w-full p-5 font-medium rtl:text-right text-gray-500 border border-b-1 border-gray-200 focus:ring-4 focus:ring-gray-200 hover:bg-gray-100 gap-3 rounded-t-xl ${isExpanded ? '' : 'rounded-b-xl'}`}>
      ...
      {isExpanded ? <div className="text-gray-500 p-5 border border-t-0 border-gray-200 rounded-b-xl">{item.description}</div> : null}
    </div>
  );
};
